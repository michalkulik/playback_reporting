...
namespace Jellyfin.Plugin.PlaybackReporting
{
    public class EventMonitorEntryPoint : IHostedService, IDisposable
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            Task.Run(() => PlaybackMonitoringTask(token));
        ...
        public async Task PlaybackMonitoringTask(CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.Error("PlaybackMonitoringTask Exception", err);
                ...
                try
                {
                    await Task.Delay(thread_sleep * 1000, cancellationToken).ConfigureAwait(false);
                }
        ...
        private PlaybackInfo GetPlaybackInfo(SessionInfo session)
        {
            ...
            string session_playing_id = session.NowPlayingItem.Id.ToString("N");
            ...
            else
            {
                ...
                {
                    transcode_reasons = session.TranscodingInfo.TranscodeReasons.ToString();
                }
                ...
                playback_info.ItemType = session.NowPlayingItem.Type.ToString();
                playback_info.RemoteAddress = session.RemoteEndPoint;
        ...
        private string GetItemName(BaseItemDto item)
        {
            ...
            else if (item.Type == BaseItemKind.Audio)
            {
                ...
                if (item.AlbumArtists != null && item.AlbumArtists.Length > 0)
                {
